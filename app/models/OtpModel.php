...
class OtpModel {
    public static function generate(string $email, int $userId=null, string $purpose='google_verify'): string {
        Database::query('UPDATE otps SET used=TRUE WHERE email=$1 AND purpose=$2 AND used=FALSE',[$email,$purpose]);
        $otp = str_pad((string)random_int(0,999999),6,'0',STR_PAD_LEFT);
        $expiry = date('Y-m-d H:i:s', time() + (int)env('OTP_EXPIRY_MINUTES',10)*60);
        Database::query(
            'INSERT INTO otps (user_id,email,otp_code,purpose,expires_at) VALUES ($1,$2,$3,$4,$5)',
            [$userId,$email,password_hash($otp,PASSWORD_DEFAULT),$purpose,$expiry]
        );
        return $otp;
    }
    public static function verify(string $email, string $code, string $purpose='google_verify'): bool {
        ...
        Database::query('UPDATE otps SET used=TRUE WHERE id=$1',[$rec['id']]);
        return true;
    }
}
