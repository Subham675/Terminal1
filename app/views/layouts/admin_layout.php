...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      --gold:#C8860A;--gold-lt:#E8A820;--dark:#111009;--char:#1A1814;--sidebar:#141210;--cream:#F6F1E8;--text:#2D2A24;--muted:#7A7264;--success:#2da44e;--danger:#cf222e;--warn:#d29922;
      --font-display:'Playfair Display', Georgia, serif;
      --font-body:'DM Sans', system-ui, -apple-system, sans-serif;
      --text-xs: clamp(0.7rem, 0.65rem + 0.2vw, 0.75rem);
      --text-sm: clamp(0.8rem, 0.76rem + 0.2vw, 0.85rem);
      --text-base: clamp(0.92rem, 0.88rem + 0.25vw, 1rem);
      --text-lg: clamp(1.1rem, 1rem + 0.35vw, 1.25rem);
      --text-xl: clamp(1.35rem, 1.25rem + 0.5vw, 1.55rem);
      --text-2xl: clamp(1.75rem, 1.5rem + 1vw, 2.2rem);
    }
    html, body{max-width:100%;overflow-x:clip}
    body{font-family:var(--font-body);background:#0F0E0B;color:#e0dbd0;display:flex;min-height:100vh;-webkit-font-smoothing:antialiased}
    /* SIDEBAR */
    .sidebar{width:240px;background:var(--sidebar);border-right:1px solid rgba(200,134,10,.12);display:flex;flex-direction:column;flex-shrink:0;position:fixed;top:0;bottom:0;left:0;overflow-y:auto;z-index:90;transition:transform .25s ease}
    .sidebar-logo{padding:28px 24px 20px;border-bottom:1px solid rgba(255,255,255,.06)}
    .sidebar-logo h1{font-family:var(--font-display);font-size:var(--text-lg);color:var(--gold-lt);letter-spacing:1px}
    .sidebar-logo small{color:rgba(255,255,255,.3);font-size:var(--text-xs);letter-spacing:2px;display:block;margin-top:2px}
    ...
    /* MAIN */
    .main{margin-left:240px;flex:1;display:flex;flex-direction:column;max-width:calc(100% - 240px);min-width:0;overflow-x:clip}
    .topbar{background:var(--char);border-bottom:1px solid rgba(255,255,255,.06);padding:14px 28px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:10;width:100%}
    ...
    th{background:rgba(255,255,255,.04);font-size:var(--text-xs);letter-spacing:1.5px;text-transform:uppercase;color:rgba(255,255,255,.35);padding:12px 16px;text-align:left}
    td{padding:12px 16px;border-bottom:1px solid rgba(255,255,255,.04);font-size:var(--text-sm);color:rgba(255,255,255,.7)}
  ...
  <div class="topbar">
    ...
    <div class="topbar-right">
      <form method="POST" action="<?= url('/auth/logout') ?>" style="display:inline">
        ...
        <button type="submit" class="btn-logout" style="background:none;border:none;cursor:pointer;font:inherit;">Logout</button>
      </form>
    </div>
  </div>
  <div class="content">
